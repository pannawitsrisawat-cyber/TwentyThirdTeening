						...
						if diff then 
							...
							if field == self.MessageQueue[i] then 						
								...
								Spawn(function()
									...
									-- ORRH Modifications Start
									--if label == field['Message'] then 
									if label == field['Message'] or (label == field['Player'] and label.TextColor3 == Color3.new(0,0,0)) then 
									-- ORRH Modifications End
...
function Chat:CreateMessage(cPlayer, message)		
	...
	if not cPlayer then 
		pName = ''
	-- ORRH Modifications Start
	elseif cPlayer == "GameMessage" then
		pName = "(ROBLOX)"
	-- ORRH Modifications End
	else 
		pName = cPlayer.Name			
	...
	--else 
		...
		local pColor 
		-- ORRH Modifications Start
		if cPlayer == "GameMessage" then
			pLabel.TextColor3 = Color3.new(0,0,0)
			pLabel.TextStrokeColor3 = Color3.new(0.7,0.7,0.7)
		--if cPlayer.Neutral then  
		elseif cPlayer.Neutral then
		-- ORRH Modifications End
			pLabel.TextColor3 = Chat:ComputeChatColor(pName)
		else 
			pLabel.TextColor3 = cPlayer.TeamColor.Color 
		...
		-- ORRH Modifications Start
		if cPlayer == "GameMessage" then
			mLabel.TextColor3 = Color3.new(0,0,0)
			mLabel.TextStrokeColor3 = Color3.new(0.7,0.7,0.7)
		end
		-- ORRH Modifications End
...
-- ORRH Modifications Start
function Chat:GameAnnounce(message)
	if PlayersService.ClassicChat then
		Chat:UpdateChat("GameMessage", message)
	end
end
-- ORRH Modifications End
...
function Chat:Initialize()			
	...
	-- ORRH Modifications Start
	if true then
		PlayersService.GameAnnounce:connect(function(message)
			Chat:GameAnnounce(message)
		end)
	end
	-- ORRH Modifications End
